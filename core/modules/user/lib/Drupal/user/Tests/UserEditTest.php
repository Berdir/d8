...
class UserEditTest extends WebTestBase {
  ...
  function testUserEdit() {
    ...
    $this->assertRaw(t('The name %name is already taken.', array('%name' => $edit['name[0][value]'])));

    // Check that filling out a single password field does not validate.
    $edit = array();
    $edit['pass[0][value][pass1]'] = '';
    $edit['pass[0][value][pass2]'] = $this->randomName();
    $this->drupalPostForm("user/" . $user1->id() . "/edit", $edit, t('Save'));
    $this->assertText(t("The specified passwords do not match."), 'Typing mismatched passwords displays an error message.');

    $edit['pass[0][value][pass1]'] = $this->randomName();
    ...
    $edit = array();
    $edit['mail[0][value]'] = $this->randomName() . '@new.example.com';
    $this->drupalPostForm("user/" . $user1->id() . "/edit", $edit, t('Save'));
    ...
    $edit = array();
    $edit['pass[0][value][pass1]'] = $new_pass = $this->randomName();
