...
class Roles extends PrerenderList {
  ...
  public function preRender(&$values) {
    ...
    if ($uids) {
      $roles = user_roles();
      $result = $this->database->query('SELECT u.entity_id as uid, u.roles_target_id as rid FROM {user__roles} u WHERE u.entity_id IN (:uids) AND u.roles_value IN (:rids)', array(':uids' => $uids, ':rids' => array_keys($roles)));
