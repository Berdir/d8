...
class Roles extends PrerenderList {
  ...
  public function preRender(&$values) {
    ...
    if ($uids) {
      $roles = user_roles();
      $result = $this->database->query('SELECT u.entity_id as uid, u.target_id as rid FROM {user__roles} u WHERE u.entity_id IN (:uids) AND u.target_id IN (:rids)', array(':uids' => $uids, ':rids' => array_keys($roles)));
