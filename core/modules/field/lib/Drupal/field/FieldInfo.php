...
class FieldInfo {
  ...
  public function getFieldMap() {
    ...
    foreach (config_get_storage_names_with_prefix('field.field') as $config_id) {
      $field_config = config($config_id)->get();
      if ($field_config['active'] && $field_config['storage']['active']) {
        $fields[$field_config['field_name']] = $field_config;
      }
    }
    // Get field instances.
    foreach (config_get_storage_names_with_prefix('field.instance') as $config_id) {
      $instance_config = config($config_id)->get();
      $field_name = $instance_config['field_name'];
      // Filter out instances of inactive fields, and instances on unknown
      // entity types.
      if (isset($fields[$field_name])) {
        $map[$field_name]['bundles'][$instance_config['entity_type']][] = $instance_config['bundle'];
        $map[$field_name]['type'] = $fields[$field_name]['type'];
      }
    }
