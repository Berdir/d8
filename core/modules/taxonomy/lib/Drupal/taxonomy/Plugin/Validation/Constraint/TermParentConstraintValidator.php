...
class TermParentConstraintValidator extends ConstraintValidator {
  ...
  public function validate($field_item, Constraint $constraint) {
    if ($field_item) {
      ...
      if ($parent_term_id && !\Drupal::entityManager()->getStorageController('taxonomy.term')->loadByProperties(array('tid' => $parent_term_id, 'vid' => $field_item->getEntity()->vid->value))) {
