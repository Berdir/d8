...
class TermTest extends TaxonomyTestBase {
  ...
  function testTermReorder() {
    ...
    $edit = array(
      'tid:' . $term2->id() . ':0[tid]' => $term2->id(),
      'tid:' . $term2->id() . ':0[parent]' => 0,
      'tid:' . $term2->id() . ':0[depth]' => 0,
      'tid:' . $term2->id() . ':0[weight]' => 0,
      'tid:' . $term3->id() . ':0[tid]' => $term3->id(),
      'tid:' . $term3->id() . ':0[parent]' => $term2->id(),
      'tid:' . $term3->id() . ':0[depth]' => 1,
      'tid:' . $term3->id() . ':0[weight]' => 1,
      'tid:' . $term1->id() . ':0[tid]' => $term1->id(),
      'tid:' . $term1->id() . ':0[parent]' => 0,
      'tid:' . $term1->id() . ':0[depth]' => 0,
      'tid:' . $term1->id() . ':0[weight]' => 2,
    );
    ...
    $this->assertEqual($terms[0]->id(), $term2->id(), 'Term 2 was moved above term 1.');
    $this->assertEqual($terms[1]->parents, array($term2->id()), 'Term 3 was made a child of term 2.');
    $this->assertEqual($terms[2]->id(), $term1->id(), 'Term 1 was moved below term 2.');
