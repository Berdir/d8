...
class ImageFieldDisplayTest extends ImageFieldTestBase {
  ...
  function _testImageFieldFormatters($scheme) {
    ...
    $this->assertLinkByHref(\Drupal::url('entity.image_style.list'), 0, 'Link to image styles configuration is found');
    ...
    $this->drupalPostAjaxForm(NULL, array(), "{$field_name}_settings_edit");
    $this->assertNoLinkByHref(\Drupal::url('entity.image_style.list'), 'Link to image styles configuration is absent when permissions are insufficient');
