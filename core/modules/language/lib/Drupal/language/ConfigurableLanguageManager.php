...
class ConfigurableLanguageManager extends LanguageManager implements ConfigurableLanguageManagerInterface {
   ...
   *
   * @todo Remove this and just set the current language onto the config
   *   factory. See https://drupal.org/node/2098119.
   */
  public function initConfigOverrides() {
    $service_id = 'locale_config_subscriber';
    if (\Drupal::getContainer()->has($service_id)) {
      $subscriber = \Drupal::service($service_id);
      \Drupal::service('event_dispatcher')->addSubscriberService($service_id, get_class($subscriber));
    }
  }
  ...
  function disableLanguageTypes(array $types) {
    $this->languageTypes['configurable'] = array_diff($this->getLanguageTypes(), $types);
    $this->saveLanguageTypesConfiguration($this->languageTypes);
  }
