...
class LanguageNegotiator implements LanguageNegotiatorInterface {
   ...
   * {@inheritdoc}
   */
  public function setContext(AccountInterface $current_user, Request $request) {
  ...
  public function initializeType($type) {
    ...
    if ($this->currentUser && $this->request) {
      ...
      foreach ($this->getConfiguration($type) as $method_id => $info) {
        ...
        $language = !empty($this->negotiatedLanguages[$method_id]) ? clone($this->negotiatedLanguages[$method_id]) : FALSE;
