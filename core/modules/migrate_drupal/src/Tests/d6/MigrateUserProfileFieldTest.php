...
class MigrateUserProfileFieldTest extends MigrateDrupalTestBase {
  ...
  public function testUserProfileFields() {
    ...
    $field = entity_load('field_storage_config', 'user.profile_color');
    ...
    // Migrated a textarea.
    $field = entity_load('field_storage_config', 'user.profile_biography');
    $this->assertEqual($field->type, 'text_long', 'Field type is text_long.');

    // Migrated checkbox field.
    $field = entity_load('field_storage_config', 'user.profile_sell_address');
    $this->assertEqual($field->type, 'list_boolean', 'Field type is list_boolean.');

    // Migrated selection field.
    $field = entity_load('field_storage_config', 'user.profile_sold_to');
    $this->assertEqual($field->type, 'list_text', 'Field type is list_text.');

    // Migrated list field.
    $field = entity_load('field_storage_config', 'user.profile_bands');
    $this->assertEqual($field->type, 'text', 'Field type is text.');
    $this->assertEqual($field->cardinality, -1, 'List field has correct cardinality');

/*
    // Migrated URL field.
    $field = entity_load('field_storage_config', 'user.profile_blog');
    $this->assertEqual($field->type, 'link', 'Field type is link.');
*/

    // Migrated date field.
    $field = entity_load('field_storage_config', 'user.profile_birthdate');
