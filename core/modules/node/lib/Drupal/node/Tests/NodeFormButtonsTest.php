...
class NodeFormButtonsTest extends NodeTestBase {
  ...
  function testNodeFormButtons() {
    ...
    // Get the node.
    $node_1 = node_load(1);
    $this->assertEqual(1, $node_1->status->value, 'Node is published');
    ...
    $this->assertEqual(0, $node_1->status->value, 'Node is unpublished');
    ...
    $node_2 = node_load(2);
    $this->assertEqual(1, $node_2->status->value, 'Node is published');
    ...
    $this->assertEqual(0, $node_2->status->value, 'Node is unpublished');
    ...
    $node_2 = node_load(2, TRUE);
    $this->assertEqual(0, $node_2->status->value, 'Node is still unpublished');
    $this->drupalLogout();
    ...
    $this->drupalPost('node/add/article', $edit, t('Save'));
    $node_3 = node_load(3);
    $this->assertEqual(0, $node_3->status->value, 'Node is unpublished');
  }
