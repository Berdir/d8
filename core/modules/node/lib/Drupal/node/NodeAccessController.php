...
class NodeAccessController extends EntityAccessController {
  ...
  /**
   * Overrides EntityAccessController::viewAccess().
   */
  public function viewAccess(EntityInterface $node, $langcode = LANGUAGE_DEFAULT, User $account = NULL) {
    ...
    $status = (bool) $node->get('status', $langcode);
    return $this->setCache($status, $node, 'view', $langcode, $account);
  }

  /**
   * Overrides EnttiyAccessController::access().
   */
  protected function access(EntityInterface $node, $operation, $langcode = LANGUAGE_DEFAULT, User $account = NULL) {
    ...
    if ($operation == 'view' && !$node->get('status', $langcode) && user_access('view own unpublished content', $account)) {
      $account = $account ?: user_load($GLOBALS['user']->uid);
      if ($account->id() != 0 && $account->id() == $node->get('uid', $langcode)) {
        return TRUE;
      }
    }
