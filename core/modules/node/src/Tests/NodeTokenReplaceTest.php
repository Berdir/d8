...
class NodeTokenReplaceTest extends TokenReplaceUnitTestBase {
  ...
  function testNodeTokenReplacement() {
    ...
    $base_cacheable_metadata = CacheableMetadata::createFromObject($node);

    $metadata_tests = [];
    $metadata_tests['[node:nid]'] = $base_cacheable_metadata;
    $metadata_tests['[node:vid]'] = $base_cacheable_metadata;
    $metadata_tests['[node:type]'] = $base_cacheable_metadata;
    $metadata_tests['[node:type-name]'] = $base_cacheable_metadata;
    $metadata_tests['[node:title]'] = $base_cacheable_metadata;
    $metadata_tests['[node:body]'] = $base_cacheable_metadata;
    $metadata_tests['[node:summary]'] = $base_cacheable_metadata;
    $metadata_tests['[node:langcode]'] = $base_cacheable_metadata;
    $metadata_tests['[node:url]'] = $base_cacheable_metadata;
    $metadata_tests['[node:edit-url]'] = $base_cacheable_metadata;
    $cacheable_metadata = clone $base_cacheable_metadata;
    $metadata_tests['[node:author]'] = $cacheable_metadata->addCacheTags(['user:1']);
    $metadata_tests['[node:author:uid]'] = $cacheable_metadata;
    $metadata_tests['[node:author:name]'] = $cacheable_metadata;
    $cacheable_metadata = clone $base_cacheable_metadata;
    $metadata_tests['[node:created:since]'] = $cacheable_metadata->setCacheMaxAge(0);
    $metadata_tests['[node:changed:since]'] = $cacheable_metadata;
    ...
    foreach ($tests as $input => $expected) {
      ...
      $output = $this->tokenService->replace($input, array('node' => $node), array('langcode' => $this->interfaceLanguage->getId()), $cacheable_metadata);
