...
class LoadTest extends FileHookTestBase {
  ...
  function testLoadMissingFilepath() {
    $files = entity_load_by_properties('file', array('uri' => 'foobar://misc/druplicon.png'));
  ...
  function testLoadInvalidStatus() {
    $files = entity_load_by_properties('file', array('status' => -99));
  ...
  function testMultiple() {
    ...
    // Load by path.
    file_test_reset();
    $by_path_files = entity_load_by_properties('file', array('uri' => $file->uri));
    $this->assertFileHookCalled('load');
