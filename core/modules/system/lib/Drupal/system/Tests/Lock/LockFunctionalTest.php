...
class LockFunctionalTest extends WebTestBase {
  ...
  public function testLockAcquire() {
    ...
    $this->assertTrue(\Drupal::lock()->acquire('system_test_lock_acquire'), 'Lock acquired by this request.', 'Lock');
    $this->assertTrue(\Drupal::lock()->acquire('system_test_lock_acquire'), 'Lock extended by this request.', 'Lock');
    ...
    $this->assertTrue(\Drupal::lock()->acquire('system_test_lock_acquire', 0.5), 'Lock acquired by this request.', 'Lock');
    sleep(1);
    ...
    $this->assertFalse(\Drupal::lock()->acquire('system_test_lock_acquire'), 'Lock cannot be extended by this request.', 'Lock');
    ...
    $this->assertTrue(\Drupal::lock()->acquire('system_test_lock_exit'), 'Lock acquired by this request after the other request exits.', 'Lock');
  }
}
