...
class EntityTranslationTest extends WebTestBase {
  ...
  function testMultilingualProperties() {
    ...
    $entities = entity_load_by_properties('entity_test', array('name' => $name));
    ...
    // explicit parameter.
    $entities = entity_load_by_properties('entity_test', array('name' => $properties[$langcode]['name'], 'default_langcode' => 0));
    $this->assertEqual(count($entities), 1, 'One entity correctly loaded by name translation.');
    $entities = entity_load_by_properties('entity_test', array('langcode' => $default_langcode, 'name' => $name));
    $this->assertEqual(count($entities), 1, 'One entity correctly loaded by name and language.');
    $entities = entity_load_by_properties('entity_test', array('langcode' => $langcode, 'name' => $properties[$langcode]['name']));
    $this->assertEqual(count($entities), 0, 'No entity loaded by name translation specifying the translation language.');
    $entities = entity_load_by_properties('entity_test', array('langcode' => $langcode, 'name' => $properties[$langcode]['name'], 'default_langcode' => 0));
    $this->assertEqual(count($entities), 1, 'One entity loaded by name translation and language specifying to look for translations.');
    $entities = entity_load_by_properties('entity_test', array('uid' => $properties[$langcode]['uid'], 'default_langcode' => NULL));
