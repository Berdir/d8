 ...
 * operation is complete by calling \Drupal::lock()->release(), as in the
 * example.
 *
 ...
 * it may call \Drupal::lock()->wait() if the rest of the current page request
 * requires that the operation in question be complete. After
 * \Drupal::lock()->wait() returns, the function may again attempt to acquire
 * the lock, or may simply allow the page request to proceed on the assumption
 * that a parallel request completed the operation.
 *
 * \Drupal::lock()->acquire() and \Drupal::lock()->wait() will automatically
 * break (delete) a lock whose duration has exceeded the timeout specified when
 * it was acquired.
 */
