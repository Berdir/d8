...
/**
 * Manages a table mapping for an entity storage.
 */
class DefaultTableMapping implements TableMappingInterface {
   ...
   */
  protected $storageDefinitions = array();
   ...
   *
   * This corresponds to the return value of
   * TableMappingInterface::getColumnMapping() except that this variable is
  ...
  public function __construct(array $storage_definitions) {
    $this->storageDefinitions = $storage_definitions;
  }
  ...
  public function getAllColumns($table_name) {
    if (!isset($this->allColumns[$table_name])) {
      ...
      foreach ($this->getFieldNames($table_name) as $field_name) {
        $this->allColumns[$table_name] = array_merge($this->allColumns[$table_name], array_values($this->getColumnMapping($field_name)));
      }
   ...
   * @return $this
   */
  public function addExtraColumns($table_name, array $column_names) {
