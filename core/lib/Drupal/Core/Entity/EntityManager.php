...
class EntityManager extends PluginManagerBase implements EntityManagerInterface {
  ...
  public function getFieldDefinitions($entity_type_id, $bundle) {
    if (!isset($this->fieldDefinitionsByBundle[$entity_type_id][$bundle])) {
      ...
      if ($cache = $this->cache->get($cid)) {
        ...
        // Rebuild the definitions and put it into the cache.
        $field_definitions_by_bundle = $this->buildFieldDefinitionsByBundle($entity_type_id, $bundle);
        $this->cache->set($cid, $field_definitions_by_bundle, Cache::PERMANENT, array('entity_types' => TRUE, 'entity_field_info' => TRUE));
      }
      ...
      $this->fieldDefinitionsByBundle[$entity_type_id][$bundle] = array_replace($this->getBaseFieldDefinitions($entity_type_id), $field_definitions_by_bundle);
    }
   ...
   *
   * @return \Drupal\Core\Field\FieldDefinitionInterface[]
   *   An array of field definitions for the bundle, keyed by field name. Does
   *   not include base fields.
   */
  protected function buildFieldDefinitionsByBundle($entity_type_id, $bundle) {
    $entity_type = $this->getDefinition($entity_type_id);
    $class = $entity_type->getClass();

    // Allow the entity class to override the base fields.
    $base_field_definitions = $this->getBaseFieldDefinitions($entity_type_id);
    $field_definitions = $class::fieldDefinitionsByBundle($entity_type, $bundle, $base_field_definitions);

    // Invoke 'per bundle' hook.
    $result = $this->moduleHandler->invokeAll('entity_field_info_by_bundle', array($entity_type, $bundle, $base_field_definitions));
    ...
    $this->moduleHandler->alter('entity_field_info_by_bundle', $field_definitions, $entity_type, $bundle);
