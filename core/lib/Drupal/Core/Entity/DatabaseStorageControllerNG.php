...
class DatabaseStorageControllerNG extends DatabaseStorageController {
   ...
   *
   * @param array $records
   *   Associative array of entity records, keyed on the entity ID.
   * @param boolean $load_revision
   *   (optional) TRUE if the revision should be loaded, defaults to FALSE.
   */
  protected function attachPropertyData(&$records, $load_revision = FALSE) {
    if ($this->dataTable) {
      ...
      foreach ($data as $values) {
        ...
        foreach ($this->entityInfo['schema_fields_sql']['data_table'] as $name) {
          $records[$id]->{$name}[$langcode][0]['value'] = $values[$name];
        }
      }
    }
  }
