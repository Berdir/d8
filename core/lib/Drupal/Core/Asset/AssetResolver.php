...
class AssetResolver implements AssetResolverInterface {
  ...
  public function getCssAssets(AttachedAssetsInterface $assets, $optimize) {
    ...
    $cid = 'css:' . $theme_info->getName() . ':' . \Drupal::request()->getScheme() . ':' . Crypt::hashBase64(serialize($libraries_to_load)) . (int) $optimize;
  ...
  public function getJsAssets(AttachedAssetsInterface $assets, $optimize) {
    ...
    $libraries_to_load = $this->getLibrariesToLoad($assets);
    $cid = 'js:' . $theme_info->getName() . ':' . \Drupal::request()->getScheme() . ':' .  $this->languageManager->getCurrentLanguage()->getId() . ':' . Crypt::hashBase64(serialize($libraries_to_load)) . (int) (count($assets->getSettings()) > 0) . (int) $optimize;
