...
/**
 * @file
 * Contains \Drupal\Component\Utility\Browser.
 */

namespace Drupal\Component\Utility;

/**
 * Provides Browser-related utility functions.
 */
class Browser {

  /**
   * Identifies browser language from the Accept-language HTTP header.
   *
   * The algorithm works as follows:
   * - map browser language codes to available language codes.
   ...
   *   (optional) Custom mappings to support browsers that are sending non
  ...
  public static function getLangcode($http_accept_language, $langcodes, $mappings = array()) {
    ...
    if (preg_match_all('@(?<=[, ]|^)([a-zA-Z-]+|\*)(?:;q=([0-9.]+))?(?:$|\s*,\s*)@', trim($http_accept_language), $matches, PREG_SET_ORDER)) {
      foreach ($matches as $match) {
        if ($mappings) {
          ...
          foreach ($mappings as $browser_langcode => $standard_langcode) {
    ...
    foreach ($browser_langcodes as $langcode => $qvalue) {
      ...
      if (!empty($generic_tag) && !isset($browser_langcodes[$generic_tag])) {
        ...
        // defined by both the browser and us.
