...
class Drupal {
   ...
   *
   * Method to coordinate long-running operations across requests.
   *
   ...
   *   if (\Drupal::lock()->acquire('mymodule_long_operation')) {
   *     // Do the long operation here.
   *     // ...
   *     \Drupal::lock()->release('mymodule_long_operation');
   *   }
   * }
   * @endcode
   *
   * If a function acquires a lock it should always release it when the operation
   * is complete by calling \Drupal::lock()->release(), as in the example.
   *
   * A function that has acquired a lock may attempt to renew a lock (extend the
   * duration of the lock) by calling \Drupal::lock()->acquire() again during the
   * operation. Failure to renew a lock is indicative that another request has
   * acquired the lock, and that the current operation may need to be aborted.
   *
   * If a function fails to acquire a lock it may either immediately return, or it may
   * call \Drupal::lock()->wait() if the rest of the current page request requires that
   * the operation in question be complete. After \Drupal::lock()->wait() returns, the
   * function may again attempt to acquire the lock, or may simply allow the page
   * request to proceed on the assumption that a parallel request completed the operation.
   *
   * \Drupal::lock()->acquire() and \Drupal::lock()->wait() will automatically break (delete)
   * a lock whose duration has exceeded the timeout specified when it was acquired.
   *
